              ...
              @foreach ($post->comments->take(3) as $comment)
                  <li class="list-group-item border-0 p-0 mb-2">
                      ...
                      <form action="{{ route('comment.destroy', $comment->id) }}" method="post">
                          ...
                          {{-- <span
                            class="text-uppercase text-muted xsmall">{{ date('M d, Y', strtotime($comment->created_at)) }}
                        </span> --}}

                          <span class="text-muted xsmall" title="{{ $comment->created_at }}">
                          ...
                          @if (Auth::user()->id === $comment->user->id)
                              &middot;
                              <button type="submit" class="border-0 bg-transparent text-danger p-0 xsmall">Delete
                                  </button:>
                          @endif
                      </form>
                  </li>
